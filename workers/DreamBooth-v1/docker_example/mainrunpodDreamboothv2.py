...
def mdlv2b():
    ...
    if os.path.exists('stable-diffusion-v2-512'):
        call('rm -r stable-diffusion-v2-512', shell=True)
    call('mkdir stable-diffusion-v2-512', shell=True)
    ...
    call('echo -e "\nscheduler\ntext_encoder\ntokenizer\nunet\nvae\nfeature_extractor\nmodel_index.json\n!*.safetensors" > .git/info/sparse-checkout',
         shell=True, stdout=open('/dev/null', 'w'), stderr=open('/dev/null', 'w'))
    call('git pull origin main', shell=True, stdout=open(
...
def mdlv2():
    ...
    os.chdir('/workspace')
    print('[1;33mDownlading the V2-768 model...')
    if os.path.exists('stable-diffusion-v2-768'):
        call('rm -r stable-diffusion-v2-768', shell=True)
    call('mkdir stable-diffusion-v2-768', shell=True)
...
def downloadmodel_pthv2(CKPT_Path, Custom_Model_Version, safetensors):
    ...
    if os.path.exists(str(CKPT_Path)):
        ...
        if Custom_Model_Version == '512':
            ...
            call('python convertodiffv2.py '+CKPT_Path +
                 ' stable-diffusion-custom --v2 --reference_model stabilityai/stable-diffusion-2-1-base '+sftnsr, shell=True)
            ...
            call('python convertodiffv2.py '+CKPT_Path +
                 ' stable-diffusion-custom --v2 --reference_model stabilityai/stable-diffusion-2-1 '+sftnsr, shell=True)
...
def downloadmodel_lnkv2(CKPT_Link, Custom_Model_Version, safetensors):
    ...
    import wget
    os.chdir('/workspace')
    call("gdown --fuzzy " + CKPT_Link + " -O "+modelnm, shell=True)

    if os.path.exists(modelnm):
        if os.path.getsize(modelnm) > 1810671599:
            if Custom_Model_Version == '512':
                call('wget -q -O convertodiffv2.py https://github.com/TheLastBen/fast-stable-diffusion/raw/main/Dreambooth/convertodiffv2.py', shell=True)
                call('python convertodiffv2.py '+modelnm +
                     ' stable-diffusion-custom --v2 --reference_model stabilityai/stable-diffusion-2-1-base '+sftnsr, shell=True)
...
def dlv2(Path_to_HuggingFace, CKPT_Path, CKPT_Link, Model_Version, Custom_Model_Version, safetensors):

    if Path_to_HuggingFace != "":
        downloadmodel_hfv2(Path_to_HuggingFace)
        MODEL_NAMEv2 = "/workspace/stable-diffusion-custom"
    elif CKPT_Path != "":
        ...
        if Model_Version == "512":
            if not os.path.exists('/workspace/stable-diffusion-v2-512/unet/config.json'):
                mdlv2b()
            MODEL_NAMEv2 = "/workspace/stable-diffusion-v2-512"
            print('[1;32mUsing the original V2-512 model')
        elif Model_Version == "768":
            if not os.path.exists('/workspace/stable-diffusion-v2-768/unet/config.json'):
                mdlv2()
            MODEL_NAMEv2 = "/workspace/stable-diffusion-v2-768"
...
def sessv2(Session_Name, Session_Link_optional, Model_Version, MODEL_NAMEv2):
    ...
    if Session_Link_optional != "":
        ...
        if Session_Link_optional != "":
            ...
            gdown.download_folder(url=Session_Link_optional, output=Session_Name,
    ...
    if os.path.exists(str(SESSION_DIR)) and not os.path.exists(MDLPTH):
        ...
        if Model_Version == '512':
            call('wget -q -O config.yaml https://github.com/Stability-AI/stablediffusion/raw/main/configs/stable-diffusion/v2-inference.yaml', shell=True)
            call('python /workspace/diffusers/scripts/convert_original_stable_diffusion_to_diffusers.py --checkpoint_path ' +
                 MDLPTH+' --dump_path '+OUTPUT_DIR+' --original_config_file config.yaml', shell=True)
            clear_output()
        elif Model_Version == '768':
            call('wget -q -O config.yaml https://github.com/Stability-AI/stablediffusion/raw/main/configs/stable-diffusion/v2-inference-v.yaml', shell=True)
            call('python /workspace/diffusers/scripts/convert_original_stable_diffusion_to_diffusers.py --checkpoint_path '+MDLPTH+' --dump_path ' +
                 OUTPUT_DIR+' --original_config_file config.yaml --upcast_attention --prediction_type v_prediction --image_size 768', shell=True)
            clear_output()
...
def uplder(Remove_existing_instance_images, Crop_images, Crop_size, IMAGES_FOLDER_OPTIONAL, INSTANCE_DIR, CAPTIONS_DIR, ren):
    ...
    def up(Upload):
        with out:
            uploader.close()
            Upload.close()
            upld(Remove_existing_instance_images, Crop_images, Crop_size,
                 IMAGES_FOLDER_OPTIONAL, INSTANCE_DIR, CAPTIONS_DIR, uploader, ren)
            done()
    out = widgets.Output()
...
def upld(Remove_existing_instance_images, Crop_images, Crop_size, IMAGES_FOLDER_OPTIONAL, INSTANCE_DIR, CAPTIONS_DIR, uploader, ren):
    ...
    if ren:
        ...
        for filename in tqdm(os.listdir(INSTANCE_DIR), bar_format='  |{bar:15}| {n_fmt}/{total_fmt} Renamed'):
            ...
            call('mv "'+os.path.join(INSTANCE_DIR, filename) +
                 '" "'+new_path_with_file+'"', shell=True)
            i = i+1
...
def caption(CAPTIONS_DIR, INSTANCE_DIR):
    ...
    def Caption(path):
        if path != "Select an instance image to caption":
            ...
            text_area = widgets.Textarea(value=text, description='', disabled=False, layout={
...
def dbtrainv2(Resume_Training, UNet_Training_Steps, UNet_Learning_Rate, Text_Encoder_Training_Steps, Text_Encoder_Concept_Training_Steps, Text_Encoder_Learning_Rate, Offset_Noise, Resolution, MODEL_NAME, SESSION_DIR, INSTANCE_DIR, CONCEPT_DIR, CAPTIONS_DIR, External_Captions,  INSTANCE_NAME, Session_Name, OUTPUT_DIR, PT, resumev2, Save_Checkpoint_Every_n_Steps, Start_saving_from_the_step, Save_Checkpoint_Every):
    ...
    def train_only_unet(stp, stpsv, SESSION_DIR, MODELT_NAME, INSTANCE_DIR, OUTPUT_DIR, Text_Encoder_Training_Steps, PT, Seed, Resolution, ofstnse, extrnlcptn, precision, Training_Steps):
        clear_output()
        if resuming == "Yes":
            print('[1;32mResuming Training...[0m')
    ...
    if Enable_text_encoder_training:
        ...
        dump_only_textenc(trnonltxt, MODELT_NAME, INSTANCE_DIR, OUTPUT_DIR,
                          PT, Seed, precision, Training_Steps=stptxt)
    ...
    if Enable_Text_Encoder_Concept_Training:
        if os.path.exists(CONCEPT_DIR):
            if os.listdir(CONCEPT_DIR) != []:
                ...
                dump_only_textenc(trnonltxt, MODELT_NAME, CONCEPT_DIR, OUTPUT_DIR,
                                  PT, Seed, precision, Training_Steps=stptxtc)
            else:
                clear_output()
                if resuming == "Yes":
                    print('[1;32mResuming Training...[0m')
    ...
    if UNet_Training_Steps != 0:
        train_only_unet(stp, stpsv, SESSION_DIR, MODELT_NAME, INSTANCE_DIR, OUTPUT_DIR, Text_Encoder_Training_Steps,
                        PT, Seed, Resolution, ofstnse, extrnlcptn, precision, Training_Steps=UNet_Training_Steps)
    ...
    if UNet_Training_Steps == 0 and Text_Encoder_Concept_Training_Steps == 0 and Text_Encoder_Training_Steps == 0:
        ...
        if os.path.exists(OUTPUT_DIR+'/unet/diffusion_pytorch_model.bin'):
            ...
            call('python /workspace/diffusers/scripts/convertosdv2.py --fp16 ' +
                 OUTPUT_DIR+' '+SESSION_DIR+'/'+Session_Name+'.ckpt', shell=True)
            clear_output()
...
def hf(Name_of_your_concept, hf_token_write, INSTANCE_NAME, OUTPUT_DIR, Session_Name, MDLPTH):
    ...
    def bar(prg):
        clear_output()
        br = "[1;33mUploading to HuggingFace : " '[0m|' + \
            '█' * prg + ' ' * (25-prg)+'| ' + str(prg*4) + "%"
        return br
    ...
    def rem(d):
        with out:
            if s.value is not None:
                clear_output()
                print("[1;33mTHE SESSION [1;31m"+s.value +
                      " [1;33mHAS BEEN REMOVED FROM THE STORAGE")
...
def crop_image(im, size):
    ...
    def focal_point(im, settings):
        corner_points = image_corner_points(
            im, settings) if settings.corner_points_weight > 0 else []
        entropy_points = image_entropy_points(
            im, settings) if settings.entropy_points_weight > 0 else []
